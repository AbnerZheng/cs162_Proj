...
load (const char *file_name, void (**eip) (void), void **esp)
{
  ...
  // 设置栈
  // 首先是放入 actual_file_name
  int total_len = 0;
  int len = strlen (actual_file_name);
  *esp -= len + 1; // 栈push一个参数
  memcpy (*esp, actual_file_name, len + 1); // 加上'\0'
  total_len += len;
  ...
  *argv = 0; // 首先放置一个0
  // 此时，*esp下面开始为argv
  char *index = down_bound;
  int argc = 1;
  argv -= 1;
  *argv = index;
//  hex_dump (*esp - 60, *esp - 60, 100, true);
  while (index <(char *) PHYS_BASE - 2) {
    if (index[0] == '\0') {
      argv -= 1;
      *argv = index + 1;
      argc += 1;
